...
import { CreateVariableInputSchema, CreateVariableToolInputSchema } from '../../src/mcp/tools/variable';
...
test('创建变量 MCP 参数 Schema 保持可导出的对象结构', () => {
    assert.equal(CreateVariableToolInputSchema instanceof Object, true);
    assert.deepEqual(Object.keys(CreateVariableToolInputSchema.shape), ['id', 'type', 'value', 'name', 'scope']);
});
