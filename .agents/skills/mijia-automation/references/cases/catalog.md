...
| 触发、状态、否则与互为状态 | state-and-scope, aggregation-and-thresholds | video / ui-sample | 来源日志显示事件未等待后续状态；需独立复现，修复结果和未知值语义待验证 |
| 虚拟事件通知闭环 | adapters-and-templates, loops-and-lifecycle | video / conflicted | App 创建、规则消费、规则产生和通知往返需分层验证 |
| 模式游标与外部状态不同步 | synchronization, loops-and-lifecycle | video | 状态查询链优先；旧值、异常档位和补脉冲风险待验证 |
| 空触点与枚举多选 | aggregation-and-thresholds, adapters-and-templates | video | 保存格式、集合边沿、重启持久性均不可写成硬规则 |
| 节律照明与动态窗口 | time-and-duration, numeric-transforms, loops-and-lifecycle | video / conflicted | 原公式超调；初始化、夹紧、跨段和跨天恢复缺失 |
