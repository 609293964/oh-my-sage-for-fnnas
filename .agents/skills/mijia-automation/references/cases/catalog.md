...
| 触发、状态、否则与互为状态 | state-and-scope, aggregation-and-thresholds | partial runtime | 已实测事件不回补及 false 走 unmet；未知值和互为状态整案待验证 |
| 虚拟事件通知闭环 | adapters-and-templates, loops-and-lifecycle | partial runtime | 规则产生和消费已实测；App 创建、通知往返和重复触发待验证 |
| 模式游标与外部状态不同步 | synchronization, loops-and-lifecycle | video | 状态查询链优先；旧值、异常档位和补脉冲风险待验证 |
| 空触点与枚举多选 | aggregation-and-thresholds, adapters-and-templates | partial runtime | modeSwitch 空档占轮次已实测；逻辑空输入、集合边沿和重启持久性待验证 |
| 节律照明与动态窗口 | time-and-duration, numeric-transforms, loops-and-lifecycle | partial runtime / conflicted | 日期时间函数和 min 夹紧已实测；初始化、跨段和跨天恢复缺失 |
